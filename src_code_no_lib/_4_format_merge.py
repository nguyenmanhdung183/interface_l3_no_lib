...
#     with open(file_path, "w", encoding="utf-8") as f:
#         #f.write("\n".join(new_lines) + "\n")
#         f.write(remove_extra_blank_lines(f"\n".join(new_lines) + "\n", keep=1))
...
def indent_c_code(file_path, tab_size=1):
    ...
    for line in lines:
        ...
        while (
            current_indent < len(stripped_line)
            and
            stripped_line[current_indent] == "\t"
        ):
    ...
    output = remove_extra_blank_lines(
        output,
        keep=2
    )
...
def remove_extra_blank_lines(text, keep=1):
    """
    Gom nhiều dòng trống liên tiếp thành keep dòng.
